...
namespace Dile.Metadata 
{
	...
	{
		uint GetAssemblyProps(uint mda, out IntPtr ppbPublicKey, out uint pcbPublicKey, out uint pulHashAlgId, [MarshalAs(UnmanagedType.LPArray, SizeParamIndex = 5)]char[] szName, uint cchName, out uint pchName, ref AssemblyMetadata pMetaData, out uint pdwAssemblyFlags);

		uint GetAssemblyRefProps(uint mdar, out IntPtr ppbPublicKeyOrToken, out uint pcbPublicKeyOrToken, [MarshalAs(UnmanagedType.LPArray, SizeParamIndex = 4)]char[] szName, uint cchName, out uint pchName, ref AssemblyMetadata pMetaData, out IntPtr ppbHashValue, out uint pcbHashValue, out uint pdwAssemblyRefFlags);
