	...
	public class CustomListView : ListView
	{
		...
		private string GetChosenMenuText(object sender)
		{
			...
			if (menuItem.Tag == null)
			{
				...
				{
					ListViewItem selectedItem = SelectedItems[itemIndex];

					for (int subItemIndex = 0; subItemIndex < selectedItem.SubItems.Count; subItemIndex++)
					{
						result.Append(selectedItem.SubItems[subItemIndex].Text);

						if (subItemIndex < selectedItem.SubItems.Count - 1)
						{
							result.Append("\t");
						}
					}
			...
			else
			{
				...
				for (int itemIndex = 0; itemIndex < SelectedItems.Count; itemIndex++)
				{
					...
					result.Append(selectedItem.SubItems[sourceColumn.Index].Text);
				}
			}
